"""
gesture_engine.py
Background gesture-detection engine optimized for Windows environment.
...
Includes transparent PyQt overlay and system mouse input simulation.
"""
...
# Optional pynput mouse injection with Windows Native fallback
try:
...
def _detect_display_geometry() -> tuple[int, int, int]:
    """Detect connected primary display resolution and orientation angle on Windows."""
    if sys.platform == "win32":
        try:
            import ctypes
            user32 = ctypes.windll.user32
...
class DisplayManager:
    ...
    def __init__(self):
        ...
        win_w, win_h, win_rot = _detect_display_geometry()
        if win_w > 0 and win_h > 0:
            self.target_width = win_w
            self.target_height = win_h
            self.rotation = win_rot
    ...
    def update(self) -> bool:
        w, h, _ = _detect_display_geometry()
        if w == 0 or h == 0:
            return False
        with self._lock:
...
class GestureState:
    ...
    def skel_anchor_x(self) -> float:
        with self._lock:
            return self._skel_anchor_x

    @property
    def skel_anchor_y(self) -> float:
        with self._lock:
            return self._skel_anchor_y
    ...
    def snapshot(self) -> dict:
        with self._lock:
            return {
                ...
                'world_landmarks': list(self._world_landmarks),
    ...
    def update(self, x: int, y: int, state: str, gesture: str, hand_detected: bool = False,
               ...
               hand_size: float = 1.0, depth_z: float = 1.0,
               skel_anchor_x: Optional[float] = None, skel_anchor_y: Optional[float] = None,
               hand_pose_mode: Optional[str] = None, palm_normal: Optional[Tuple[float, float, float]] = None) -> None:
        with self._lock:
            self._cursor_x = x
            self._cursor_y = y
            ...
            if skel_anchor_x is not None:
                self._skel_anchor_x = skel_anchor_x
            if skel_anchor_y is not None:
                self._skel_anchor_y = skel_anchor_y
            if hand_pose_mode is not None:
                self._hand_pose_mode = hand_pose_mode
            if palm_normal is not None:
                self._palm_normal = palm_normal

    def set_pose_normal(self, hand_pose_mode: str, palm_normal: Tuple[float, float, float]) -> None:
        with self._lock:
            self._hand_pose_mode = hand_pose_mode
            self._palm_normal = palm_normal
...
class GestureProcessor:
    ...
    def _extract_lm_coords(self, lms: Any) -> Optional[np.ndarray]:
        if lms is None:
            return None
        coords = []
        try:
            ...
            if len(coords) < 21:
                return None
            return np.nan_to_num(np.array(coords, dtype=np.float32))
        except Exception:
            return None

    def process(self, lms: Any, world_lms: Optional[Any] = None, handedness: str = "Right") -> None:
        now = time.perf_counter()
        raw_lm_np = self._extract_lm_coords(lms)
        if raw_lm_np is None:
            self.miss()
            return
        world_lm_np = self._extract_lm_coords(world_lms) if world_lms else None
    ...
    def _run_pipeline(self, now: float, raw_lm_np: Optional[np.ndarray], world_lm_np: Optional[np.ndarray], handedness: str = "Right") -> None:
        ...
        if raw_lm_np is not None:
            ...
            self._state.set_pose_normal(hand_pose_mode, palm_normal)
        ...
        publish_world_lm = world_lm_np.tolist() if world_lm_np is not None else []
        ...
        depth_val = float(np.clip(self._depth_z_ema, 0.65, 1.8))
        ...
        self._state.update(
            ...
            world_landmarks=publish_world_lm,
            hand_size=depth_val,
            depth_z=depth_val,
            skel_anchor_x=float(skel_sx),
            skel_anchor_y=float(skel_sy),
        )

        if ENABLE_SYSTEM_MOUSE:
    ...
    def reset(self) -> None:
        ...
        self._state.update(
            ...
            hand_size=1.0,
            depth_z=1.0,
            skel_anchor_x=float(self._state.x),
            skel_anchor_y=float(self._state.y),
        )
...
class CameraStream:
    ...
    def start(self) -> bool:
        ...
        if sys.platform == "win32" and not self.cap.isOpened():
            # Fallback backend if DirectShow is unavailable
    ...
    with _mp_vision.HandLandmarker.create_from_options(options) as landmarker:
        ...
        while not stop_event.is_set():
            try:
                    ...
                    while True:
                        ...
                        if lm:
                            try:
                                ...
                                log.warning("Landmark processing exception: %s", proc_err)
                                try:
                                    processor.miss()
                                except Exception:
                                    pass
                ...
                log.error("Camera stream error: %s", e)
                if cap is not None:
                    try:
                        cap.release()
                    except Exception:
